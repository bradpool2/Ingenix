...
function Pago() {
    ...
    const confirmarPago = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Alerta con los productos comprados
                ...
                alert("Error al procesar el pago: " + (data.message || "intenta de nuevo"));
            }
        } catch (err) {
            console.error(err);
    ...
    return (
        <>
            <NavBar />
            <div className="contenedor-padre" style={{ alignItems: "flex-start", paddingTop: "100px" }}>
                <div className="tarjeta-login tarjeta-pago">
                    ...
                    {paso === 1 && (
                        <>
                            ...
                            <button className="btn-pago-secundario" onClick={() => navigate("/carrito")}>
                                Volver al carrito
                            </button>
                        </>
                    )}
                        ...
                        <>
                            ...
                            <div className="lista-metodos">
                                {METODOS.map((m) => (
                                    <div
                                        key={m.id}
                                        className={`metodo-item ${metodoSeleccionado === m.id ? "metodo-activo" : ""
                                            }`}
                                        onClick={() => {
                                    ...
                                    >
                                        <div
                                            className={`radio-circulo ${metodoSeleccionado === m.id ? "radio-activo" : ""
                                                }`}
                                        />
                            ...
                            {metodoInfo?.campo && (
                                <div className="campo-detalle-pago">
                                    ...
                                    <input
                                        type="text"
                                        className={`input-detalle ${errorDetalle ? "input-error" : ""}`}
                                        ...
                                        maxLength={metodoInfo.campo === "tarjeta" ? 19 : 10}
                                    />
                                    {errorDetalle && <span className="error-detalle">{errorDetalle}</span>}
                                </div>
                            )}

                            {errorDetalle && !metodoInfo?.campo && (
                            ...
                            <button className="btn-pago-secundario" onClick={() => setPaso(1)}>
                                Volver
                            </button>
                        </>
                    )}
                    ...
                    {paso === 3 && (
                        <div className="confirmacion-pago">
                            ...
                            <p>Método: <strong>{metodoSeleccionado}</strong></p>
                            {referenciaPago && (
                                <p style={{ fontSize: "13px", color: "#777" }}>
                                    Referencia: <strong>{referenciaPago}</strong>
                                </p>
                            )}
                            <div className="lista-productos-pago" style={{ marginTop: "16px" }}>
                                {carrito.length > 0
                                    ? carrito.map(item => (
                                        <div key={item.idProducto} className="item-pago">
                                    ...
                                    : <p className="subtexto-pago">Compra registrada</p>
                                }
                            </div>
                            <button className="btn-pago-principal" onClick={() => navigate("/home")}>
                                Volver al inicio
                            </button>
                            <button className="btn-pago-secundario" onClick={() => navigate("/catalogo")}>
                                Seguir comprando
                            </button>
                        </div>
                    )}
